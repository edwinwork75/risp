...
import { GripVertical, Trash2, Copy, Type, CheckSquare, ChevronDown, ToggleLeft, Calendar, Clock, File, Hash, Phone, FileImage } from "lucide-react";
...
export function FieldEditor({ field, onUpdate, onDelete, onDuplicate, isActive, onClick }: FieldEditorProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const fieldTypes: FieldType[] = [
    'text', 'number', 'phone', 'dropdown', 'radio', 'checkbox', 'date', 'time', 'file', 'image'
  ];

  return (
    <div ref={setNodeRef} style={style} {...attributes} className="relative group" onClick={onClick}>
      <Card className={cn("mb-3 border-l-[6px]", { "border-primary": isActive, "border-transparent": !isActive })}>
